...
export class RatioGridStyler {
  ...
  constructor(
    ...
    this.baseItemWidth = this._getBaseItemSize(
      this.percentWidthPerItem,
      this.gutterWidthFractionPerItem
    );

    this.percentHeightPerItem = this.percentWidthPerItem / this.rowHeightRatio;
    this.baseItemHeight = `${this._rowHeight}px`;
    //this.baseItemHeight =  this._getBaseItemSize(
    //   this.percentHeightPerItem,
    //   this.gutterWidthFractionPerItem
    // );
  }
  ...
  private _getItemPosition(baseSize: string, offset: number): string {
    ...
    return offset === 0
      ? '0'
      : calc(`(${baseSize} + ${this._gutterSize}px) * ${offset}`);
  }
  ...
  private _setColStyles(tile: HTMLElement, colIndex: number, span: number) {
    ...
    this._setStyle(
      tile,
      'left',
      this._getItemPosition(this.baseItemWidth, colIndex)
    );
    this._setStyle(
      tile,
      'width',
      calc(this._getItemSize(this.baseItemWidth, span))
    );
  }

  private _setRowStyles(
    tile: HTMLElement,
    rowIndex: number,
    rowspan: number
  ): void {
    ...
    this._setStyle(
      tile,
      'marginTop',
      this._getItemPosition(this.baseItemHeight, rowIndex)
    );
    this._setStyle(
      tile,
      'paddingTop',
      calc(this._getItemSize(this.baseItemHeight, rowspan))
    );
  }
